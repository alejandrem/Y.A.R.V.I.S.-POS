...
3. admintarvis/chat.rs delega en src-ia/motor-chat:
   - Cloud: cloud/apis_cloud/generacion.rs abre un stream HTTP/SSE a Gemini. Los bloques think/response se separan (think.rs). Si Gemini devuelve 429, espera 2-4 s y reintenta (luego cae al local). Las 19 tools se resuelven via ciclo_tools.rs con re-inyeccion del resultado como mensaje role tool (MAX_RONDAS_TOOLS=3).
