...
3. Se registran 144 comandos Tauri en el invoke_handler y se inicia el job de alertas cada hora.
...
5. El LLM no carga al arrancar. Se carga lazy cuando el chat lo necesita (load_chat_model), y el motor local comparte una unica instancia Qwen2.5-Coder 1.5B Instruct fine-tuneado.
...
3. admintarvis/chat.rs delega en src-ia/motor-chat:
   - Cloud: cloud/apis_cloud/generacion.rs abre un stream HTTP/SSE a Gemini o NVIDIA (transporte OpenAI-compatible). Los bloques think/response se separan (think.rs). Si Gemini devuelve 429, espera 2-4 s y reintenta (luego cae al local). Las 19 tools se resuelven via ciclo_tools.rs con re-inyeccion del resultado como mensaje role tool (MAX_RONDAS_TOOLS=3).
