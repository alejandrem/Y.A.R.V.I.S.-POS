...
const API_PROVIDERS: { id: ProviderId; name: string; description: string; placeholder: string }[] = [
  ...
  { id: "nvidia", name: "NVIDIA", description: "Modelos NIM (build.nvidia.com), OpenAI-compatibles", placeholder: "nvapi-…" },
];
...
const PanelYarvis = ({ rol, active = true }: PanelYarvisProps) => {
  ...
  const [configSection, setConfigSection] = useState<"google" | "nvidia" | "local">("google");
  ...
  const [selectedProvider, setSelectedProvider] = useState<"" | ProviderId>(() => {
    ...
    return stored === "google" || stored === "nvidia" ? stored : "";
  });
  ...
  const currentSelection: ChatModelSelection = useMemo(() => {
    if (selectedProvider) {
      const providerName = selectedProvider === "google" ? "Gemini" : "NVIDIA";
  ...
  const saveApiConfig = async () => {
    if (rol === "admin") {
      ...
      // Al pegar key sin modelo elegido: se preselecciona el default
      // verificado automáticamente (no más menú vacío).
      let next = { ...selectedCloudModels };
      let ultimo: ProviderId | null = null;
      (["google", "nvidia"] as ProviderId[]).forEach((pid) => {
        const def = CLOUD_PROVIDERS.find((p) => p.id === pid)?.defaultModel;
        if (apiKeys[pid]?.trim() && !next[pid] && def) {
          next = { ...next, [pid]: def };
          localStorage.setItem(`yarvis_cloud_model_${pid}`, def);
          ultimo = pid;
        }
      });
      if (ultimo) {
        setSelectedCloudModels(next);
        localStorage.setItem("yarvis_cloud_models_selected", JSON.stringify(next));
        setSelectedProvider(ultimo);
        localStorage.setItem("yarvis_active_provider", ultimo);
      }
    ...
    <div className="yarvis-shell flex h-full min-h-0 flex-col animate-in fade-in duration-500">
      ...
      {showConfig && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
        <div className="yarvis-shell yarvis-panel yarvis-border yarvis-shadow max-h-[90vh] w-full max-w-2xl overflow-hidden rounded-3xl border">
          ...
          <div className="flex gap-2 overflow-x-auto px-6 pt-5 sm:px-8">{[{ id: "google", label: "Gemini" }, { id: "nvidia", label: "NVIDIA" }, { id: "local", label: "Modelo local" }].map((item) => <button key={item.id} onClick={() => setConfigSection(item.id as typeof configSection)} className={`flex-shrink-0 rounded-xl px-4 py-2.5 text-[10px] font-black uppercase tracking-widest ${configSection === item.id ? "yarvis-primary" : "yarvis-panel-soft yarvis-muted"}`}>{item.label}</button>)}</div>
